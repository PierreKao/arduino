...
UBYTE 	ucSPVol = 15; 							// Speaker volume, MAX=31 MIN=0	
	...
	if(nLD_Mode == LD_MODE_ASR_RUN) {
		...
		if((ucRegVal & 0x10) && LD_ReadReg(0xb2)==0x21 && LD_ReadReg(0xbf)==0x35) {
			...
			if(nAsrResCount>0 && nAsrResCount<=4) {
				printf("ASR SUCCESSFUL \r\n");
				nAsrStatus = LD_ASR_FOUNDOK;
			}
			else {
				printf("ASR UNSUCCESSFUL \r\n");
...
UBYTE LD_ASR(void)
{
	...
	for (i=0; i<5; i++) {						//run ASR try 5 times
		...
		Driver_Delay_ms(100);
		if (LD_AsrAddKey() == 0) {		//Add fixed to LD3320
			printf("ERROR!!! LD_AsrAddKey\r\n");
